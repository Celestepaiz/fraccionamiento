...
export default class MaintenanceList extends Component {
    ...
    componentDidMount(){
        axios.get('http://10.50.65.22:3000/api/all-access')
             .then((response) => {
    ...
    //id
    deleteHandler = (index) => {
        axios.delete(`http://10.50.65.22:3000/api/access/${this.state.data[index][4]}`)
             .then(() => {
                const dataCopy = [
                    ...this.state.data
                ]                 
    ...
    updateHandler = (index) => {
        this.props.history.push({
            pathname: '/update-access',
            state: {
                codigo: this.state.data[index][4]
            }
        })
    }
